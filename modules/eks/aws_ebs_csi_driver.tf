# Creating an IAM OIDC Provider for IRSA
...
# Attaching official policy to this role
resource "aws_iam_role_policy_attachment" "ebs_irsa_policy" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"
  role       = aws_iam_role.ebs_csi_irsa_role.name
}

# Update the latest compatible version to match the k8s version in the cluster
...
# EKS Addon with an associated IRSA IAM role
